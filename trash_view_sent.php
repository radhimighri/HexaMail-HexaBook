...
if (!isset($_GET['id_message_sent']) || empty($_GET['id_message_sent'])) {
  ...
  $sql = 'SELECT titre, date, message, membre.login as destinateur FROM msg_sent, membre WHERE id_expediteur="'.$_SESSION['id'].'"   AND id_destinataire=membre.id AND msg_sent.id="'.$_GET['id_message_sent'].'" AND msg_sent.trash=1 ';
